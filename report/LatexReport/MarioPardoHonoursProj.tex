...
%% PACKAGES
%% ============================================================
\usepackage[top=1in, bottom=0.5in, left=1.25in, right=1in, headheight=15pt, headsep=16pt]{geometry}
\usepackage{setspace}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
...
A brute-force point-to-point method was used first: for each point in the source cloud, the closest
...
The SVD of $W$ is given by $W = U D V^\top$. We can interpret $V$ as the directions coming \textbf{into} the transformation, $U$ as the transformed (output) direction vectors for those same inputted directions, and $D$ as the scaling for each direction. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{../images/WorkingICP.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{../images/PoseGraphDiagram.jpg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\textwidth]{../../Pictures/ProgressPics/LoopClosure}
    \caption{Visualization of a detected loop closure and the resulting graph edge connecting the
...
The improvement in map quality from using ICP over raw odometry is clearly visible.
...
a popular benchmark for 2D SLAM, recorded in a real research facility.
...
Below is the function signature for the core ICP alignment function as a reference.

% TODO: insert representative code snippet(s) here.
\vspace{1cm}
