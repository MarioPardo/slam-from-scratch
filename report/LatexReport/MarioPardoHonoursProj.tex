...
Instananeous Center of Curvature (ICC)
The exact displacement for constant-curvature motion is obtained by integrating the arc
analytically:
\begin{align}
    x'      &= x + R\bigl(\sin(\theta + \Delta\theta) - \sin\theta\bigr) \\
    y'      &= y + R\bigl(\cos\theta - \cos(\theta + \Delta\theta)\bigr)
\end{align}
This is exact for a circular arc, whereas a simple midpoint approximation would accumulate
error during sustained turns.
...
to occupy.
Accumulating these projections over time builds a persistent point-cloud map of the environment,
and any drift in the pose estimate directly manifests as smearing of the map --- making the quality
of the map a useful diagnostic for localisation accuracy.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{../images/LidarDiagram.jpeg}
    \caption{Diagram of Lidar Points}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \caption{Occupancyggrid mid-trajectory.}
...
\section{Takeaways from Results}
...
<bold> Symmetry <b> In the Crosses Map and the Ball Map we see that a highly symmetric map gives issues to ICP in localization. 
Both Apartment and Regular Maps have highly assymetrical environments, and this is seen in their performance.

<Landmarks> In both hallway maps, we saw an immediate and significant improvement in performance once landmarks were introduced. 
As we noted, ICP makes great use of landmarks to best calculate relative transformations. 

<Round Landmarks> Surprisingly, we saw that round landmarks did very well both in Ball Map and in Everythin Round. 
...
We conclude that the main weakness in the system is in it's recognition and filtering of landmarks and surfaces. 
...
\bibitem{ros2_kinematics}
ros2\_control Development Team,
``Wheeled Mobile Robot Kinematics,''
\textit{ROS\,2 Controllers Documentation (Rolling)}.
[Online]. Available: \url{https://control.ros.org/rolling/doc/ros2_controllers/doc/mobile_robot_kinematics.html}
...
\bibitem{diff_drive_icc}
Leo,
``Calculating Differential Drive Robot ICC Position,''
\textit{Robotics Stack Exchange}.
[Online]. Available: \url{https://robotics.stackexchange.com/questions/15013/calculating-differential-drive-robot-icc-position}
...
The source code can be found at \href{https://github.com/MarioPardo/slam-from-scratch}{Github Repository}
\newline
